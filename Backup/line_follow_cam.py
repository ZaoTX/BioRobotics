...
def analyze_image(image, prev_value):
    ...
    middle_pos, middle_index = find_white_pix(binary_img[-15], middle)
...
def control_car(dry_run=False):
    ...
    while not killer.kill_now:
        ...
        if np.abs(angular_v) < .8:
            angular_v *= 20
            linear_v = int(400)
        elif np.abs(angular_v) < 1.5:
            angular_v *= 30
            linear_v = int(400 - np.abs(angular_v)*0.2)
        else :
            angular_v *= 30
            linear_v = int(400 - np.abs(angular_v)*1.1)
